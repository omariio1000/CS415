...
int main(int argc, char **argv) {
    ...
    if (N % size != 0) {
        if (rank == 0) {
            printf("Data size N must be evenly divisible by the number of processes P\n");
        }
        fclose(fin);
        MPI_Finalize();
        exit(0);
    ...
    fseek(fin, rank * local_N * sizeof(int), SEEK_SET);
    ...
    for (int i = 0; i < local_N; i++) {
        int k = BktIdx(local_data[i], DATABITS, num_buckets);
    ...
    if (rank == 0) recv_counts_prefix_sum[rank] = 0;
