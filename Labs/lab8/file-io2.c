...
int main(int argc, char *argv[])
{
    ...
    int buf[localN + (rank < remainder ? 1 : 0)];

    int offset = rank * localN + (rank < remainder ? rank : remainder);
    int count = localN + (rank < remainder ? 1 : 0);

    MPI_File_read_at(fin, offset * sizeof(int), buf, count, MPI_INT, MPI_STATUS_IGNORE);
    ...
    printf("P%d: block=%d, items=%d, psum = %d\n", rank, count, offset, localSum);
